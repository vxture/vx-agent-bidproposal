# ADR-006: bid carries a real business domain - the 20-Second Challenge

- **Status:** accepted
- **Date:** 2026-08-31 (records an owner decision: make bid a usable product
...
Meanwhile ADR-001 already committed bid to being a deployed product, on the
...
- **Rigid zone untouched.** Gating formulas, C1/C2/C3 semantics, usage
  ...
  unchanged. The game meters `bid.game.runs` through the same buffered
  counter path as `bid.chat.messages`.
...
- The platform metric registry needs `bid.game.runs` (and optionally the
  `bid.game.runs_per_day` limit key) - liaison letter 130.
