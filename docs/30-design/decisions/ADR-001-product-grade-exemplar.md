# ADR-001: bid is a deployed product, not a placeholder skeleton
...
- **Supersedes:** the placeholder-cascade model described in the pre-2026-08-16
  ...
  `80-2608131530-bid-repo-renamed.md`
...
chain - and it shipped four production releases to `bid.vxture.com`. The
...
name, `build.yml` had to run `instantiate.mjs bid` against the ephemeral CI
...
bid is a deployed Vxture product **and** the reference build new product repos
are copied from. These are the same artifact by design.

1. **No placeholders.** Every name in the tree is the literal `bid` value that
...
2. **Product creation is copy-and-rename.** `scripts/init/rename-product.mjs`
   ...
   and it refuses to rewrite bid's liaison history, which belongs to bid.
3. **The exemplar zone replaces the blank zone.** Where the template previously
   shipped an empty slot with a comment, bid ships a working implementation a
...
4. **Real integration is the acceptance bar.** bid must genuinely call Atlas and
...
- bid's liaison correspondence, batch tracker, and tech-debt register are its
