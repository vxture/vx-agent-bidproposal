...
Two columns below: what is already true for **bid**, and what a product copied
from bid has to obtain for itself. Nothing here is optional - an unregistered
...
- [x] **bid** product row in the platform product directory (`product.products`,
      `product_code='bid'`, `status='active'`). Other products can also target
      `audience=bid` because of this row.
- [ ] Plan structure (subscription tiers) seeded for the product. **Not done for
      bid**: the platform seed defines no bid plans or `plan_components`, so
      the subscription leg of the S2S coverage gate cannot be satisfied. The
      workable leg today is provisioning (below).

## OIDC (customer realm)

- [x] **bid** OIDC client registered, realm = customer, status active.
- [ ] `bid-beta` client. **Deliberately not registered** - bid is prod-only
...
- [x] **bid** registered in `product_webhooks` with its tailnet delivery
      address (`BID_WEBHOOK_BASE_URL`).
- [x] `BID_PROVISION_WEBHOOK_SECRET` live on the platform side and transported
