...
-- section 7). The runtime connects as bidproposal_svc, NOT the DB
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'bidproposal_svc') THEN
    CREATE ROLE bidproposal_svc LOGIN;
  END IF;
END $$;

GRANT USAGE ON SCHEMA vx_provision, local_authz, local_usage TO bidproposal_svc;
