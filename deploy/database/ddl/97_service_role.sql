...
-- section 7). The runtime connects as bid_svc, NOT the DB
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'bid_svc') THEN
    CREATE ROLE bid_svc LOGIN;
  END IF;
END $$;

GRANT USAGE ON SCHEMA vx_provision, local_authz, local_usage TO bid_svc;
